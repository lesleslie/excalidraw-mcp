...
from excalidraw_mcp.monitoring.health_checker import HealthStatus
...
class TestMonitoringIntegration:
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_healthy_monitoring(self, monitoring_supervisor, mock_canvas_server, mock_config):
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_failure_recovery(self, monitoring_supervisor, mock_canvas_server, mock_config):
    ...
    @pytest.mark.asyncio
    async def test_automatic_restart_integration(self, monitoring_supervisor, mock_canvas_server, mock_config):
    ...
    @pytest.mark.asyncio
    async def test_circuit_breaker_integration(self, monitoring_supervisor, mock_config):
        ...
        if cb_stats["failed_calls"] >= monitoring_supervisor.circuit_breaker._failure_threshold:
            assert monitoring_supervisor.circuit_breaker.state == CircuitState.OPEN
        
        await monitoring_supervisor.stop()

    @pytest.mark.asyncio
    async def test_metrics_collection_integration(self, monitoring_supervisor, mock_canvas_server, mock_config):
    ...
    @pytest.mark.asyncio
    async def test_alert_generation_integration(self, monitoring_supervisor, mock_config):
        ...
        alerts = monitoring_supervisor.get_recent_alerts()
    ...
    @pytest.mark.asyncio
    async def test_callback_integration(self, monitoring_supervisor, mock_canvas_server, mock_config):
    ...
    @pytest.mark.asyncio
    async def test_resource_monitoring_integration(self, monitoring_supervisor, mock_canvas_server, mock_config):
    ...
    @pytest.mark.asyncio
    async def test_request_tracing_integration(self, monitoring_supervisor, mock_config):
        """Test request tracing integration across HTTP client and monitoring."""
        # Mock HTTP client with tracing
        with patch('excalidraw_mcp.monitoring.health_checker.http_client') as mock_client:
            mock_client.check_health = AsyncMock(return_value=True)
            mock_client.get_json = AsyncMock(return_value=[])
            mock_client.get_request_metrics = Mock(return_value={
                "total_requests": 10,
    ...
    @pytest.mark.asyncio
    async def test_graceful_shutdown_integration(self, monitoring_supervisor, mock_canvas_server, mock_config):
    ...
    @pytest.mark.asyncio
    async def test_performance_under_load(self, monitoring_supervisor, mock_canvas_server, mock_config):
