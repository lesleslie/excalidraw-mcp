...
class TestMonitoringIntegration:
    ...
    async def test_end_to_end_failure_recovery(
        ...
        # supervisor's restart success path resets consecutive_failures; the
        # restart attempt is the unambiguous signal that failures were detected)
        status = monitoring_supervisor.get_monitoring_status()
    ...
    async def test_performance_under_load(
        ...
        # Also disable resource monitoring because the production code calls
        # `psutil.Process(...).cpu_percent(interval=0.1)` which blocks the
        # event loop for 100ms per cycle; this is unrelated to loop cadence
