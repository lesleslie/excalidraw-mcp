...
class TestProcessManagerCoverage:
    ...
    async def test_ensure_running_auto_start_disabled(self, process_manager):
        ...
        with patch('excalidraw_mcp.process_manager.config') as mock_config, \
            ...
            mock_logger.warning.assert_called_once_with("Canvas server not running and auto-start is disabled")

    @pytest.mark.asyncio
    async def test_ensure_running_start_process_failure(self, process_manager):
        """Test ensure_running when starting process fails."""
        with patch.object(process_manager, '_is_process_healthy', return_value=False), \
    ...
    async def test_start_process_exception(self, process_manager):
        ...
        with patch.object(process_manager, '_get_project_root', side_effect=Exception("Test exception")), \
             patch('excalidraw_mcp.process_manager.logger') as mock_logger:
            ...
            assert "Failed to start canvas server: Test exception" in str(mock_logger.error.call_args[0])

    @pytest.mark.asyncio
    async def test_start_process_success(self, process_manager):
        """Test _start_process when it succeeds."""
        with patch.object(process_manager, '_get_project_root'), \
    ...
    async def test_wait_for_health_timeout(self, process_manager):
        ...
        with patch.object(process_manager, '_is_process_running', return_value=True), \
            ...
            with patch('excalidraw_mcp.process_manager.config') as mock_config:
                ...
                # Should return False
                assert result is False
                mock_logger.error.assert_called_once_with("Canvas server failed to become healthy within timeout")

    @pytest.mark.asyncio
    async def test_wait_for_health_process_dies(self, process_manager):
        """Test _wait_for_health when process dies during waiting."""
        with patch.object(process_manager, '_is_process_running', side_effect=[True, False]), \
             patch('excalidraw_mcp.process_manager.logger') as mock_logger:
            ...
            # Should return False
            assert result is False
            mock_logger.error.assert_called_once_with("Canvas server process died during startup")
