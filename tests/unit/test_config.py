...
import os
import pytest
from excalidraw_mcp.config import Config, SecurityConfig, ServerConfig, PerformanceConfig, LoggingConfig
...
class TestConfig:
    ...
    def test_validation_negative_values(self):
        ...
        try:
            with pytest.raises(ValueError, match="Max elements per canvas must be positive"):
                Config()
        finally:
