...
class CanvasHTTPClient:
    ...
    async def check_health(self, force: bool = False) -> bool:
        ...
        if (not force and 
            current_time - self._health_cache.timestamp < config.server.health_check_interval_seconds):
        ...
        async with self._lock:
            ...
            try:
                ...
                if is_healthy:
                    ...
                    logger.warning(f"Canvas server health check failed: HTTP {response.status_code}")
    ...
    async def post_json(self, endpoint: str, data: Dict[str, Any], retries: int = None) -> Optional[Dict[str, Any]]:
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"Canvas server returned HTTP {response.status_code}: {response.text}")
                    if attempt < retries:
                ...
                logger.warning(f"Canvas server request timeout (attempt {attempt + 1}/{retries + 1})")
                if attempt < retries:
                    await asyncio.sleep(config.server.sync_retry_delay_seconds)
                    continue
                return None
    ...
    async def put_json(self, endpoint: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Canvas server PUT returned HTTP {response.status_code}: {response.text}")
                return None
    ...
    async def get_json(self, endpoint: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Canvas server GET returned HTTP {response.status_code}: {response.text}")
                return None
