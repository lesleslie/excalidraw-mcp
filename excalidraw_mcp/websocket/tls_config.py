"""TLS configuration for Excalidraw-mcp WebSocket server.
...
def get_websocket_tls_config() -> dict[str, str | bool | None]:
    ...
    Environment Variables:
        EXCALIDRAW_WS_TLS_ENABLED: Enable TLS ("true" or "false")
        EXCALIDRAW_WS_CERT_FILE: Path to certificate file (PEM format)
        EXCALIDRAW_WS_KEY_FILE: Path to private key file (PEM format)
        EXCALIDRAW_WS_CA_FILE: Path to CA file (for client verification)
        EXCALIDRAW_WS_VERIFY_CLIENT: Verify client certificates ("true" or "false")

    Returns:
        Dictionary with TLS configuration
    """
    return get_tls_config_from_env("EXCALIDRAW_WS")
