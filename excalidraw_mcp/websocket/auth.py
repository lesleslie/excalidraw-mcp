"""WebSocket authentication configuration for Excalidraw-MCP.

This module provides JWT authentication configuration for the Excalidraw-MCP
...
JWT_SECRET = os.getenv("EXCALIDRAW_JWT_SECRET", "dev-secret-change-in-production")

# Get token expiry from environment (default: 1 hour)
TOKEN_EXPIRY = int(os.getenv("EXCALIDRAW_TOKEN_EXPIRY", "3600"))

# Check if authentication is enabled
AUTH_ENABLED = os.getenv("EXCALIDRAW_AUTH_ENABLED", "false").lower() == "true"
...
def get_authenticator() -> WebSocketAuthenticator | None:
    ...
    """
    if not AUTH_ENABLED:
        logger.info("Excalidraw WebSocket authentication disabled (development mode)")
        return None

    if JWT_SECRET == "dev-secret-change-in-production":
        logger.warning(
            "Using default JWT secret - please set EXCALIDRAW_JWT_SECRET "
...
def generate_token(user_id: str, permissions: list[str] | None = None) -> str:
    ...
    return authenticator.create_token({
        "user_id": user_id,
        "permissions": permissions or ["excalidraw:read"],
    })
